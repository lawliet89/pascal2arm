...
; User Code
;		 R5 is now GLOBAL_index
	MOV R5, #1 ;Line 8
FOR_0;		 R6 is now a temporary variable
	MOV R6, #4 ;Line 9
;		 R7 is now a temporary variable
	MUL R7, R6, R5 ;Line 9
;		 R8 is now GLOBAL_myarray
	MOV R8, R5 ;Line 9
	ADD R5, R5, #1
	CMP R5, #5
	LDR R12, =GLOBAL_index; Force storage of variable
	STR R5, [R12]
	LDR R12, =GLOBAL_myarray; Load address of array start
	STR R8, [R12, R7]; Store array with offset calculated
	BLE FOR_0
;		 R9 is now GLOBAL_index
	MOV R9, #1 ;Line 11
FOR_1;		 R10 is now a temporary variable
	MOV R10, #4 ;Line 12
;		 R11 is now a temporary variable
	MUL R11, R10, R9 ;Line 12
;		 R1 is now GLOBAL_myarray
	LDR R12, =GLOBAL_myarray; Load address of array start
	LDR R1, [R12, R11]; Load array with offset calculated
