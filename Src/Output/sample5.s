...
;		 R2 is now GLOBAL_b
	MOV R2, #4 ;Line 5
;		 R3 is now a temporary variable
	MOV R3, R1 ;Line 6
	CMP R3, R2 ;Line 6
	MOVLT R3, #1 ;Line 6
	MOVGE R3, #0 ;Line 6
	CMP R3, #1
	BLT WHILE_1
WHILE_0	MOV R0, R1; Moving register temporarily
	BL PRINTR0_ ;Print integer
	ADD R1, R1, #1 ;Line 9
;		 R4 is now a temporary variable
	MOV R4, R1 ;Line 6
	CMP R4, R2 ;Line 6
	MOVLT R4, #1 ;Line 6
	MOVGE R4, #0 ;Line 6
	CMP R4, #1
	BGE WHILE_0
WHILE_1	MOV R0, R2; Moving register temporarily
	BL PRINTR0_ ;Print integer
	SWI SWI_Exit
;--------------------------------------------------------------------------------
