...
;		 R5 is now GLOBAL_a
	MOV R5, #1 ;Line 5
FOR_0	MOV R0, R5; Moving register temporarily
	BL PRINTR0_ ;Print integer
	ADD R1, R1, #1 ;Line 8
	ADD R5, R5, #1
	CMP R5, #10
	LDR R12, =GLOBAL_b; Force storage of variable
	STR R1, [R12]
	LDR R12, =GLOBAL_a; Force storage of variable
	STR R5, [R12]
	BLE FOR_0
;		 R6 is now GLOBAL_b
	LDR R12, =GLOBAL_b ;Loading variable
	LDR R6, [R12]
	MOV R0, R6; Moving register temporarily
	BL PRINTR0_ ;Print integer
	SWI SWI_Exit
;--------------------------------------------------------------------------------
