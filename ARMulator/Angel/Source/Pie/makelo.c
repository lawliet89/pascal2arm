...
int main(int argc, char **argv)
{
  ...
  timeptr = gmtime(&tval);
  if (timeptr == NULL) timeptr = localtime(&tval);
  ...
  *tptr = '\0'; /* replace newline with NULL */
  ...
  fprintf(outfile, "MMUFlushIDCReg\t\tCN\t%d\n", MMUFlushIDCReg);
  fprintf(outfile, "\nMMUOn\t\t\tEQU\t0x%08X\n", MMUOn);
  ...
  fprintf(outfile, "Angel_ApplHeapLimit\tEQU\t0x%08X\n",
          Angel_ApplHeapLimit);
#endif
  fprintf(outfile, "Angel_DefaultTopOfMemory\t\tEQU\t0x%08X\n",
          Angel_DefaultTopOfMemory);
  ...
  fprintf(outfile, "IH_SERIAL\tEQU\t%d\n", IH_SERIAL);
#if PARALLEL_SUPPORTED
  fprintf(outfile, "IH_PARALLEL\tEQU\t%d\n", IH_PARALLEL);
#endif
#if PROFILE_SUPPORTED
  fprintf(outfile, "IH_TIMER\tEQU\t%d\n", IH_TIMER);
#endif
  ...
  fprintf(outfile, "SerialChipBase\tEQU\t&%08x\n", SerialChipBase);
  fprintf(outfile, "SER_ISR\t\tEQU\t%d\n", SER_ISR);
  fprintf(outfile, "SerialIntSource\tEQU\t%d\n", SerialIntSource);
  fprintf(outfile, "IMRTimer\tEQU\t%d\n", IMRTimer);
  fprintf(outfile, "PIObase\t\tEQU\t&%08x\n", PIObase);
  fprintf(outfile, "PIO_RSR\t\tEQU\t%d\n", PIO_RSR);
  fprintf(outfile, "RxDataMask\tEQU\t%d\n", RxDataMask);
