...
/*
 * RCS $Revision: 1.108.2.11 $
 * Checkin $Date: 1998/03/10 12:52:55 $
...
Dbg_Error Dbg_Initialise(
    toolconf config, Dbg_HostosInterface const *i,
    Dbg_RDIResetCheckProc *checkreset,
    RDIProcVec const *rdi, Dbg_MCState **statep,
    rdi_onerror_fn *onerrorproc);
/* values in config are updated if they call for default values */

void Dbg_Finalise(Dbg_MCState *state, bool targetdead, rdi_onerror_fn *onerrorproc);
...
Dbg_Error Dbg_AttachDevice(
    Dbg_MCState *parent, toolconf config, Dbg_HostosInterface const *i,
    Dbg_RDIResetCheckProc *checkreset, RDIProcVec const *rdi, Dbg_MCState **statep);
...
Dbg_Error Dbg_WriteRegister(
    Dbg_MCState *state, unsigned int rno, ARMword val, unsigned int mode);
...
enum {
  Dbg_RegisterFormat_Last,      /* end of the list */
  Dbg_RegisterFormat_String,    /* string */
  Dbg_RegisterFormat_Word,      /* word as hex */
  Dbg_RegisterFormat_Bits,      /* some bits of a word */
  Dbg_RegisterFormat_Enum,      /* enumeration of strings */
  Dbg_RegisterFormat_FBits,     /* formatted bits */
  Dbg_RegisterFormat_FBitsR,    /* formatted bits (reverse) */
  Dbg_RegisterFormat_Flag,      /* bits flags following formats (non-zero first) */
  Dbg_RegisterFormat_FPRegVal   /* word is start of an FPRegVal (hack) */
  };
...
Dbg_Error Dbg_DescribeCoPro(
    Dbg_MCState *state, unsigned int cpnum, Dbg_CoProDesc *p);
...
Dbg_Error Dbg_DescribeCoPro_RDI(
    Dbg_MCState *state, int cpnum, Dbg_CoProDesc *p);
...
struct Dbg_Value {
    ...
    int f77csize;
    union {
        struct { unsigned long reg; ARMaddress frame; } r;
        ARMaddress ptr;
        Dbg_ConstantVal c;
        void *localp;
        Dbg_AccessFnRec f;
...
typedef struct {
  ...
  union {
    ...
    struct { unsigned int mode; unsigned int r; } cpu;
    struct { unsigned int no; unsigned int r; } cp;
    void *localp;
    Dbg_AccessFnRec f;
  } loc;
} Dbg_Loc;
...
typedef void Dbg_BreakAlteredProc(Dbg_MCState *state, ARMaddress addr, bool set, Dbg_BreakPosType sort);
...
extern Dbg_Error Dbg_ToolconfToConfigBlock(
    Dbg_MCState *state, toolconf config, Dbg_ConfigBlock *block);
extern Dbg_Error Dbg_ConfigBlockToToolconf(
    Dbg_MCState *state, toolconf tconf, const Dbg_ConfigBlock *config);
#endif
