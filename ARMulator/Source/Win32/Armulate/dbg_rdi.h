...
/*
 * RCS $Revision: 1.55.2.6 $
 * Checkin $Date: 1998/03/12 17:39:12 $
...
/*
 * Macros to prevent a call to a dead RDI
 */
#ifndef NODEADRDICHECKS
#define STOP_ACCESS_TO_DEAD_TARGET(xstate) if(xstate->rdi_is_dead != RDIError_NoError) return xstate->rdi_is_dead;
#define CHECK_FOR_DEAD_TARGET(xstate, xrdiReturn) if(xrdiReturn == RDIError_LinkTimeout || \
                                                     xrdiReturn == RDIError_Interrupted) \
                                                     xstate->rdi_is_dead = xrdiReturn;
#else
#define STOP_ACCESS_TO_DEAD_TARGET(xstate)
#define CHECK_FOR_DEAD_TARGET(xstate, xrdiReturn)
#endif
...
struct Dbg_MCState;
#ifdef PICCOLO
typedef int rdi_open_proc(unsigned type, toolconf config,
...
#else
typedef int rdi_execute_proc(struct Dbg_MCState **, PointHandle *handle);
typedef int rdi_step_proc(struct Dbg_MCState **, unsigned ninstr, PointHandle *handle);
#endif
...
typedef int rdi_onerror_fn(const void *device, int errtyp, void *data);
