...
;
; RCS $Revision: 1.21.2.2 $
; Checkin $Date: 1997/08/21 12:42:53 $
...
_dflt   EnterWithLR_16
    ...
    |
        ...
        ASSERT  dOPh = a1
        ;MOVEQ  dOPh, #0                ;Return zero if appropiate
        MOVEQ   dOPl, #0
        ReturnToLR EQ
...
;determine the exponent whilst normalising and rounding the value.
        ...
        ORR     tmp, tmp, #((DExp_bias+31):AND:&ff00)<<DExp_pos
      [ CodeSize = 1
        B       _dflt_shared
      |
        ORR     tmp, tmp, #((DExp_bias+31):AND:&00ff)<<DExp_pos
...
_dflt_shared
      ...
      [ CodeSize <> 2
        MOV     tmp, #((DExp_bias+31):AND:&ff00)<<DExp_pos
      |
        ORR     tmp, tmp, #((DExp_bias+31):AND:&ff00)<<DExp_pos
      ]
      [ CodeSize = 1
...
;===========================================================================
        ...
        ; Round argument to an integer towards -Inf
        ; Uses the algorithm in math.c

floor   EnterWithLR

;Test for zero
        ORRS    tmp,dOPl,dOPh,LSL #1    ; Z<-zero, C<-sign
        ReturnToLR EQ
        BCS     floor_neg

;From here on we know that the argument is +ve, hence simple truncation will
;do

        MOV     a3,dOPh,LSL #1
        MOV     a3,a3,LSR #DFhi_len+1   ; isolate exponent

        ASSERT  DExp_bias = 0x3ff
        SUB     a3,a3,#&400             ; leave with a bias of -1
        CMP     a3,#-1
        BLT     floor_return_zero
        
        CMP     a3,#DFhi_len+32+1       ; 1 for exponent bias
        ReturnToLR GE                   ; already an integer amount
        CMP     a3,#DFhi_len+1
        BGE     floor_pos_trunc_lo

; The bits we need to throw away are all of lo and some of hi

        MOV     dOPl,#0

        RSB     a3,a3,#DFhi_len-1       ; Number of bits to clear out
        MOV     dOPh,dOPh,LSR a3
        MOV     dOPh,dOPh,LSL a3
        ReturnToLR                      ; Truncation complete

floor_return_zero

;The exponent is a zero and the number is +ve - return 0.0;
...
;Only bits from the bottom word need truncating

        RSB     a3,a3,#31+DFhi_len-1
        MOV     dOPl,dOPl,LSR a3
        MOV     dOPl,dOPl,LSL a3
        ReturnToLR
...
;Enter with a non-zero -ve number
        ...
        SUB     a3,a3,#&400             ; leave with a bias of -1
        CMP     a3,#-1
        BLT     floor_return_minus_one
        
        CMP     a3,#DFhi_len+32+1       ; 1 for exponent bias
        ReturnToLR GE                   ; already an integer amount

        CMP     a3,#DFhi_len+1
        BGE     floor_neg_trunc_lo
...
; The bits we need to throw away are all of lo and some of hi
        ...
        MOV     dOPh,dOPh,LSL a3
        TEQEQ   tmp,dOPh
        ReturnToLR EQ                   ; Truncation complete
        B       floor_sub_one

floor_return_minus_one

;(-1) is 0xbff00000 0x00000000

        MOV     dOPh,#0xbff00000 :AND: 0xff000000
        ORR     dOPh,dOPh,#0xbff00000 :AND: 0x00ff0000
        MOV     dOPl,#0
        ReturnToLR
...
;Construct -1.0 and return via _dadd

        MOV     dOP2h,#0xbff00000 :AND: 0xff000000
        ORR     dOP2h,dOP2h,#0xbff00000 :AND: 0x00ff0000
        MOV     dOP2l,#0
        B       _dadd
...
;Test for zero
        ORRS    tmp,dOPl,dOPh,LSL #1    ; Z<-zero, C<-sign
        ReturnToLR EQ                   ; ceil(a zero) = zero
        BCS     ceil_neg

;From here on we know that the argument is +ve
...
ceil_return_one
        ...
        MOV     dOPh,#0x3ff00000 :AND: 0xff000000
        ORR     dOPh,dOPh,#0x3ff00000 :AND: 0x00ff0000
        MOV     dOPl,#0
        ReturnToLR
...
ceil_add_one
        ...
        MOV     dOP2h,#0x3ff00000 :AND: 0xff000000
        ORR     dOP2h,dOP2h,#0x3ff00000 :AND: 0x00ff0000
        MOV     dOP2l,#0
        B       _dadd
...
;Enter with a non-zero -ve number - simple truncation is all that is
;needed
        ...
        SUB     a3,a3,#&400             ; leave with a bias of -1
        CMP     a3,#-1
        BLT     ceil_return_zero
        
        CMP     a3,#DFhi_len+32+1       ; 1 for exponent bias
        ReturnToLR GE                   ; already an integer amount

        CMP     a3,#DFhi_len+1
        BGE     ceil_neg_trunc_lo
